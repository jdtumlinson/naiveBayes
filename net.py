...
def main():
  ...
  kaggle_pred_test = np.argmax(logits,axis=1)[:,np.newaxis]
  test_out = np.concatenate((np.expand_dims(np.array(range(4220), dtype=int), axis=1), kaggle_pred_test), axis=1)
...
class LinearLayer:
  ...
  def backward(self, grad):
    ...
    #(eq. 27) dL/dW = self.input.T @ grad -- transpose X times dL/dZ
    self.grad_weights = self.input.T @ grad
    
    #(eq. 27) dL/db = np.sum(grad) -- summation of dL/dZ
    self.grad_bias = np.sum(grad)

    #(eq.35) dL/dX = grad @ self.weights.T -- dL/dZ times transpose W
